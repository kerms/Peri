...
void loop() 
{
  ...
  if (now - lastMsg > 2000) 
  {
    ...
    //Serial.println("Value touch ok : " + String(touchRead(touch_ok)));
    if (touchRead(touch_summer) < 10) {
      snprintf (msg, 75, "touch ok : %d", touchRead(touch_summer));
    ...
    if(touch_val < 15 && touch_val>0) {
      snprintf (msg, 75, "touch not ok : %d", touch_val);
      Serial.print("Publish message: ");
      Serial.println(msg);
      client.publish(OUT_TOPIC, msg);
      lastMsg = now;
